...
class PinOrderDialog(QDialog):
    ...
    def _populate_items(self):
        ...
        for idx, pin in enumerate(self.pin_names):
            ...
            item.setBackground(
                QBrush(QColor("#f0f0f0") if idx % 2 else QColor("#ffffff"))
            )
...
class PinItem(QGraphicsEllipseItem):
    ...
    def __init__(self, pin_id: str, radius: float = PIN_DIAMETER, parent=None):
        ...
        self.setPen(QPen(QColor(SUP_COLOR), 1))
...
class _BaseConnector(QGraphicsPathItem):
    ...
    def __init__(
        self, x: float, y: float, name: str, pin_names: list[str], color=BODY_COLOR
    ):
        super().__init__()
        self.name = name
    ...
    def _paint_sup_bar(self, painter, rect: QRectF):
        ...
        painter.drawText(
            QRectF(rect.left(), y_sup, rect.width(), LABEL_HEIGHT),
            Qt.AlignCenter,
            self.name,
        )
    ...
    def addPinsToScene(self, scene, connector_id):
        ...
        if hasattr(scene, "_pin_registry"):
            ...
            to_remove = [
                k for k, v in scene._pin_registry.items() if v.parentItem() == self
            ]
            for k in to_remove:
            ...
            orphans = [
                k for k, v in scene._pin_registry.items() if v.parentItem() is None
            ]
            for k in orphans:
                del scene._pin_registry[k]
...
class LeftConnector(EditablePinOrderMixin, _BaseConnector):
    ...
    def _build_geometry(self):
        ...
        body_width = max(
            self._pin_text_width(draw_names)
            + LEFT_MARGIN
            + PIN_DIAMETER
            + MID_MARGIN
            + RIGHT_MARGIN,
        ...
        for i in range(len(draw_names)):
            self.pin_centers_local.append(
                QPointF(x_center, i * PIN_HEIGHT + PIN_HEIGHT / 2)
            )
...
class RightConnector(EditablePinOrderMixin, _BaseConnector):
    ...
    def _draw_content(self, painter):
        ...
        for idx, text in enumerate(self._draw_names):
            ...
            painter.setPen(QPen(QColor(SUP_COLOR), 1))
            painter.drawLine(
                QPointF(x_sep, y0 + 7), QPointF(x_sep, y0 + PIN_HEIGHT - 7)
            )

            # Draw pin label
            painter.drawText(
                QRectF(x_txt, y0, text_width, PIN_HEIGHT), Qt.AlignCenter, text
            )
...
class TopConnector(EditablePinOrderMixin, _BaseConnector):
    ...
    def _build_geometry(self):
        ...
        self.cell_w = [
            max(
                PIN_DIAMETER + LEFT_MARGIN + RIGHT_MARGIN,
        ...
        total_height = (
            LABEL_HEIGHT + PIN_HEIGHT + MID_MARGIN + PIN_DIAMETER + LEFT_MARGIN
        )
    ...
    def _draw_content(self, painter):
        ...
        for idx, (width, text) in enumerate(zip(self.cell_w, self._draw_names)):
            ...
            painter.setPen(QColor(SUP_COLOR))
            painter.drawText(
                QRectF(block_left, label_y, width, PIN_HEIGHT), Qt.AlignCenter, text
            )
...
class BottomConnector(EditablePinOrderMixin, _BaseConnector):
    ...
    def _draw_content(self, painter):
        ...
        for width, label in zip(self.cell_w, self._draw_names):
            # Draw pin label
            painter.drawText(
                QRectF(x, y_label, width, PIN_HEIGHT), Qt.AlignCenter, label
            )
            x += width
        ...
        for width in self.cell_w:
            painter.drawLine(
                QPointF(x + 4, separator_y), QPointF(x + width - 4, separator_y)
            )
            x += width
        ...
        for width in self.cell_w[:-1]:
            x += width
            painter.drawLine(
                QPointF(x, y_label), QPointF(x, self.rect.bottom() - LABEL_HEIGHT + 25)
            )
