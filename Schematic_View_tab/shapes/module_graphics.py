...
from Schematic_View_tab.shapes.connector_utils import (
    suggest_module_size,
    auto_distribute_connectors,
    layout_side,
    enable_side_drag,
)
...
class ModuleGraphics(QGraphicsPathItem):
    ...
    def __init__(
        ...
        if connectors_info and (
            width == DEFAULT_MODULE_WIDTH and height == DEFAULT_MODULE_HEIGHT
        ):
        ...
        self._hover_gradient = QLinearGradient(
        ...
        self._selected_gradient = QLinearGradient(
            0, 0, self._rect.width(), self._rect.height()
        )
    ...
    def _add_connectors(self, connectors_info):
        ...
        temp_connectors = [
            {
                "id": info["id"],
                "name": info["name"],
                "pin_names": info["pin_names"],
                "side": info.get("side", "top"),
                "color": info.get("color", "#F8913C"),
            }
        ...
        auto_distribute_connectors(
            self._rect.width(), self._rect.height(), dummy_connectors, spacing=10
        )

        for i, temp_info in enumerate(temp_connectors):
            side = (
                dummy_connectors[i].side
                if hasattr(dummy_connectors[i], "side")
                else temp_info["side"]
            )
            ...
            for pin_idx, pin_name in enumerate(connector.pin_names):
                ...
                pin_center_local_to_module = (
                    connector.pos() + pin_center_local_to_connector
                )
                module_key = str(getattr(self, "db_id", self._name))
                pin_uid = build_pin_uid(module_key, connector.name, pin_name)
        ...
        for connector in self.connectors:
            ...
            try:
                ...
                print(
                    f"Warning: Could not enable side drag for connector {connector.name}: {e}"
                )
            ...
            for connector in self.connectors:
                ...
                for pin_idx, pin_name in enumerate(connector.pin_names):
                    ...
                    if pin_uid in self.pins:
                        pin_center_local_to_connector = connector.get_pin_offset(
                            pin_idx
                        )
                        self.pins[pin_uid]["center_local"] = (
                            connector.pos() + pin_center_local_to_connector
                        )
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.resizing and self.resize_edge:
            ...
            if "l" in self.resize_edge:
                new_left = min(
                    new_rect.right() - actual_min_width, new_rect.left() + diff.x()
                )
                new_rect.setLeft(new_left)
            if "r" in self.resize_edge:
                new_right = max(
                    new_rect.left() + actual_min_width, new_rect.right() + diff.x()
                )
                new_rect.setRight(new_right)
            if "t" in self.resize_edge:
                new_top = min(
                    new_rect.bottom() - actual_min_height, new_rect.top() + diff.y()
                )
                new_rect.setTop(new_top)
            if "b" in self.resize_edge:
                new_bottom = max(
                    new_rect.top() + actual_min_height, new_rect.bottom() + diff.y()
                )
                new_rect.setBottom(new_bottom)

            # Only update if the new rect is valid
            if (
                new_rect.width() >= actual_min_width
                and new_rect.height() >= actual_min_height
            ):
                self._rect = new_rect
                self.updatePath()
    ...
    def _detect_edge(self, pos):
        ...
        if (
            abs(pos.x() - rect.left()) < corner_margin
            and abs(pos.y() - rect.top()) < corner_margin
        ):
            return "tl"
        if (
            abs(pos.x() - rect.right()) < corner_margin
            and abs(pos.y() - rect.top()) < corner_margin
        ):
            return "tr"
        if (
            abs(pos.x() - rect.left()) < corner_margin
            and abs(pos.y() - rect.bottom()) < corner_margin
        ):
            return "bl"
        if (
            abs(pos.x() - rect.right()) < corner_margin
    ...
    def paint(self, painter, option, widget=None):
        ...
        painter.drawLine(
            QPointF(self._rect.left() + 2, title_y),
            QPointF(self._rect.right() - 2, title_y),
        )
        pixmap = None
        if self._image_path:
        ...
        if pixmap and not pixmap.isNull():
            ...
            painter.drawPixmap(
                int(image_rect.left() + (image_rect.width() - scaled.width()) / 2),
                int(image_rect.top() + (image_rect.height() - scaled.height()) / 2),
                scaled,
            )
    ...
    def calculate_minimum_size(self, margin=36, spacing=14):
        ...
        for side in ["top", "bottom"]:
            if by_side[side]:
                total_connector_width = sum(
                    c.boundingRect().width() for c in by_side[side]
                )
        ...
        for side in ["left", "right"]:
            if by_side[side]:
                total_connector_height = sum(
                    c.boundingRect().height() for c in by_side[side]
                )
