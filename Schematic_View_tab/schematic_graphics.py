...
from Schematic_View_tab.shapes.connector_pin_graphics import (
    build_pin_uid,
    ConnectorFactory,
)
...
class SchematicGraphicsScene(QGraphicsScene):
    ...
    def _collect_active_interface_ids(self):
        ...
        try:
            if (
                hasattr(self, "interface_graphics_items")
                and self.interface_graphics_items
            ):
    ...
    def calculate_total_current(self):
        ...
        try:
            ...
            displayed_module_ids = (
                set(self.module_graphics_items.keys())
    ...
    """
                cursor.execute(
                    main_q, tuple(active_interface_ids) + (current_project_id,)
                )
    ...
    def update_display_from_selection(self, selection_dict):
        ...
        # Layout parameters for a compact, non-overlapping opening arrangement
        x_start, y_start = 90, 110
        ...
        try:
            with get_connection() as conn:
                ...
                if module_ids:
                    ...
                    cur.execute(
                        f"SELECT id FROM connectors WHERE module_id IN ({placeholders}) AND project_id = %s",
            ...
            with get_connection() as conn:
                ...
                for mod_id in module_ids:
                    ...
                    for conn_id, conn_name, conn_color, saved_side in cur.fetchall():
                        ...
                        default_color = conn_color or "#27ae60"
        ...
        # Position modules using priority: saved positions > current positions > database positions > stable grid
        placed_rects = []
        row = col = 0
        x, y = x_start, y_start
        max_row_h = 0

        def fits_position(candidate_x, candidate_y, width, height):
            candidate_rect = QRectF(candidate_x, candidate_y, width, height)
            for rect in placed_rects:
                if candidate_rect.intersects(rect):
                    return False
                if (
                    abs(candidate_rect.left() - rect.right()) < 20
                    or abs(rect.left() - candidate_rect.right()) < 20
                ):
                    if (
                        candidate_rect.top() < rect.bottom()
                        and candidate_rect.bottom() > rect.top()
                    ):
                        return False
            return True
        ...
        for data in mod_items_data:
            mod_item = data["item"]
            mod_id = data["id"]
            base_width = max(180, int(data["width"] + 50))
            base_height = max(140, int(data["height"] + 50))
            ...
            if mod_id in saved_module_positions:
                ...
                if isinstance(saved_pos, tuple) and len(saved_pos) >= 2:
                    ...
                    if fits_position(saved_x, saved_y, base_width, base_height):
                        chosen_pos = (saved_x, saved_y)
            if chosen_pos is None and mod_id in current_positions:
                current_x, current_y = (
                    current_positions[mod_id].x(),
                    current_positions[mod_id].y(),
                )
                if fits_position(current_x, current_y, base_width, base_height):
                    chosen_pos = (current_x, current_y)
            if (
                chosen_pos is None
                and data["db_pos"][0] is not None
                and data["db_pos"][1] is not None
            ):
                db_x, db_y = data["db_pos"]
                if fits_position(float(db_x), float(db_y), base_width, base_height):
                    chosen_pos = (float(db_x), float(db_y))
            if chosen_pos is None:
                candidate_x = x + MARGIN_FOR_CONNECTION
                candidate_y = y + MARGIN_FOR_CONNECTION
                if not fits_position(candidate_x, candidate_y, base_width, base_height):
                    candidate_x = (
                        x_start
                        + (col * (base_width + spacing_x))
                        + MARGIN_FOR_CONNECTION
                    )
                    candidate_y = (
                        y_start
                        + (row * (base_height + spacing_y))
                        + MARGIN_FOR_CONNECTION
                    )
                chosen_pos = (candidate_x, candidate_y)

                x += base_width + spacing_x
                max_row_h = max(max_row_h, base_height)
                col += 1
                if col >= n_cols:
                    col, x = 0, x_start
            ...
            mod_item.setPos(*chosen_pos)
            placed_rects.append(
                QRectF(chosen_pos[0], chosen_pos[1], base_width, base_height)
            )
        ...
        if saved_connector_positions:
            for conn_id, item in self.connector_graphics_items.items():
                if conn_id in saved_connector_positions:
                    saved_x, saved_y, saved_width, saved_height, saved_side = (
                        saved_connector_positions[conn_id]
                    )
                    ...
                    if hasattr(item, "side") and item.side != saved_side:
                        ...
                        if parent:
                            ...
                            if (
                                hasattr(parent, "connectors")
                                and item in parent.connectors
                            ):
                    ...
                    # Apply saved position relative to the parent module, not as an absolute scene position
                    parent = item.parentItem()
                    if parent:
        ...
        try:
            with get_connection() as conn:
                ...
                cur.execute(
                    """
                    SELECT I.id, p1.id, p1.name, c1.name, c1.module_id AS m1_id, p2.id, p2.name, c2.name, c2.module_id AS m2_id, I.color
                    FROM Interfaces I
                    JOIN pins p1 ON I.pin1_id = p1.id 
                    JOIN connectors c1 ON p1.connector_id = c1.id 
                    JOIN pins p2 ON I.pin2_id = p2.id 
        ...
        for (
            ...
            if p1_id in pin_ids and p2_id in pin_ids:
                uid1 = build_pin_uid(str(m1_id), c1_name, p1_name)
                uid2 = build_pin_uid(str(m2_id), c2_name, p2_name)
        ...
        for uid1, uid2, color_str, interface_id in connection_pairs:
            ...
            if pin1 and pin2:
                ...
                if interface_id in saved_interface_positions:
                    saved_x, saved_y, saved_rotation = saved_interface_positions[
                        interface_id
                    ]
    ...
    def refresh_connections_and_pins(self):
        ...
        with get_connection() as conn:
            ...
            cur.execute("""
                SELECT
                    p1.id AS pin1_id, p1.name AS pin1_name, c1.name AS conn1_name, c1.module_id AS mod1_id,
                    p2.id AS pin2_id, p2.name AS pin2_name, c2.name AS conn2_name, c2.module_id AS mod2_id,
        ...
        for row in rows:
            ...
            uid1 = build_pin_uid(str(mod1_id), conn1_name, pin1_name)
            uid2 = build_pin_uid(str(mod2_id), conn2_name, pin2_name)
        ...
        for uid1, uid2, color_str in connection_pairs:
            ...
            if (
                not pin1
                or not pin2
                or pin1.parentItem() is None
                or pin2.parentItem() is None
            ):
                continue
            ...
            edge = SmartOrthogonalConnector(
                ...
                obstacles=[
                    item
                    for item in self.items()
                    if hasattr(item, "db_id") and hasattr(item, "boundingRect")
                ],
                lead=20,
                margin=20,
                line_width=3,
...
class TransparentTreeSelector:
    ...
    def get_checked_ids(self):
        if hasattr(self, "tree_widget") and hasattr(
            self.tree_widget, "get_checked_ids"
        ):
...
class ZoomableGraphicsView(QGraphicsView):
    ...
    def __init__(self, scene, parent=None):
        ...
        if self.tree_selector.selectionChanged:
            self.tree_selector.selectionChanged.connect(
